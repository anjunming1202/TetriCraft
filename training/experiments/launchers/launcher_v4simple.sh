#!/bin/bash
# v4 = winner recipe (Huber+wells+H128, gamma param) + ADAPTIVE return-normalization + best-ckpt-save.
# args: RUN_NAME NODE JOBID SEED GAMMA
RUN_NAME=$1; NODE=$2; JOBID=$3; SEED=$4; GAMMA=$5
WT=/home/u6gb/junming.u6gb/tetricraft-worktree/feature-mlp-v4simple-20260812
...
for attempt in $(seq 1 300); do
  ...
  srun --jobid=$JOBID --overlap --nodes=1 --ntasks=1 -w $NODE --cpus-per-task=20 \
    ...
    python -u training/scripts/run_training.py \
      ...
      --updates-per-step 4 --lr 1e-3 --grad-clip-norm 10 --gamma $GAMMA --batch-size 512 --seed $SEED \
